...
def get_verb_pairs():
    ...
    verbs_remove = ['like', 'love', 'want', 'know', 'feel', 'try', 'think', 'try', 'may', 'get']
    for video in data.keys():
        list_actions = data[video]
        list_actions_filtered = [action for [action, sentence] in list_actions if action not in verbs_remove and len(action.split()) >= 2]
